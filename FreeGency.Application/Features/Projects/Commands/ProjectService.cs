...
public partial class ProjectService : IProjectService
{
    ...
    public ProjectService(ICurrentUserService currentUser, IUnitOfWork unitOfWork, IMapper mapper, IEntitlementService entitlementService)
    {
    ...
    public async Task<ApiResponse<Guid>> CreateAsync(CreateProjectRequestDto request, CancellationToken ct = default)
    {
        ...
        // check entitlement
        var quota = await _entitlementService.CanConsumeAsync(_currentUser.UserId, FeatureType.CreateProject, ct);
        if (!quota.IsAllowed)
            return ApiResponse.Failure<Guid>(quota.ToAppError());
