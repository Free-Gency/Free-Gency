...
public class ProjectFileRepository: GenericRepository<ProjectFile>, IProjectFileRepository
{
    ...
    {
        IQueryable<ProjectFile> query = _dbSet.AsNoTracking().Where(f => f.ProjectId == projectId);
    ...
    {
        return await _dbSet.AsNoTracking().Where(f => f.MilestoneId == milestoneId).OrderByDescending(f => f.CreatedAt).ToListAsync(ct);
    }
}