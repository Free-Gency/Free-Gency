...
public class ChatRoomRepository:GenericRepository<ChatRoom>, IChatRoomRepository
{
    ...
    {
        return await _dbSet.AsNoTracking().Where(r => r.ChatRoomMembers.Any(m => m.UserId == userId)).OrderByDescending(r => r.UpdatedAt ?? r.CreatedAt).ToListAsync(ct);
    }

    public async Task<ChatRoom?> GetTeamMainAsync(Guid teamId,CancellationToken ct = default)
    {
        return await _dbSet.AsNoTracking().FirstOrDefaultAsync(r => r.TeamId == teamId && r.RoomType == RoomType.TeamMain,ct);
    }

    public async Task<ChatRoom?> GetByProjectIdAsync(Guid projectId,CancellationToken ct = default)
    {
        return await _dbSet.AsNoTracking().FirstOrDefaultAsync(r => r.ProjectId == projectId, ct);
    }

    public async Task<ChatRoom?> GetByProposalIdAsync(Guid proposalId,CancellationToken ct = default)
    {
        return await _dbSet.AsNoTracking().FirstOrDefaultAsync(r => r.ProposalId == proposalId, ct);
    }

    public async Task AddWithMembersAsync(ChatRoom room,IEnumerable<Guid> memberUserIds,CancellationToken ct = default)
    {
        ...
        await _context
            .Set<ChatRoomMember>()
            .AddRangeAsync(members, ct);
    }

    public async Task AddMemberAsync(Guid roomId,Guid userId,CancellationToken ct = default)
    {
        var exists = await _context.Set<ChatRoomMember>().AnyAsync(x =>x.ChatRoomId == roomId && x.UserId == userId,ct);
        if (exists)
            return;
    ...
    {
        var member = await _context.Set<ChatRoomMember>().FirstOrDefaultAsync(x =>x.ChatRoomId == roomId && x.UserId == userId,ct);
        if (member is null)
            return;
