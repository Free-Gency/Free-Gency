...
public class ProjectEventRepository:GenericRepository<ProjectEvent>, IProjectEventRepository
{
    ...
    {
        return await _dbSet.AsNoTracking().Where(e => e.ProjectId == projectId).OrderByDescending(e => e.CreatedAt).Skip(skip).Take(take).ToListAsync(ct);
    }

    public async Task<ProjectEvent?> GetLatestByTypeAsync(Guid projectId,EventType eventType,CancellationToken ct = default)
    {
        return await _dbSet.AsNoTracking().Where(e => e.ProjectId == projectId &&e.EventType == eventType).OrderByDescending(e => e.CreatedAt).FirstOrDefaultAsync(ct);
    }
}